...
xm_per_pix = 4/590 # meters per pixel in x dimension
...
def abs_sobel_thresh(img, orient='x', sobel_kernel=3, thresh=(0,255)):#, gaussian_kernel=(1,1)):
...
def process_image(img):
    ...
    grady = abs_sobel_thresh(img, orient='y', thresh=(20,255)) #25
    c_binary = color_threshold(img, sthresh=(60,255), vthresh=(50,150)) # sthresh=(100,255), vthresh=(50,255)
    preprocessImage[((gradx == 1) & (grady == 1) | (c_binary == 1))] = 255 # preprocessImage[((gradx == 1) & (grady == 1) | (c_binary == 1))] = 255
    ...
    src = np.float32([[265,669], [577,460], [704,460], [1040,669]]) # short: [531,492], [755,492] / middle: [577,460], [704,460] / long: [599,445], [680,445] 
    ...
    if ((tracking_errors.distance == True or tracking_errors.direction == True or tracking_errors.curvature == True) and last_frame.first_frame == False): # or tracking_errors.lost_tracking == True):
    ...
    # check if at least one sanity check has failed in the last x frames
    ...
    #cv2.putText(result, direction + ', radius = ' + str(round(middle_curverad_m, 1)) + '(m)', (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2) # different distance
    ...
    sobel[((gradx == 1) & (grady == 1))] = 255
    gradx[(gradx == 1)] = 255
    grady[(grady == 1)] = 255
    c_binary[(c_binary == 1)] = 255 
    
    first_pic = cv2.cvtColor(sobel, cv2.COLOR_GRAY2BGR) # sobel
    second_pic = cv2.cvtColor(c_binary, cv2.COLOR_GRAY2BGR) # c_binary
    third_pic = cv2.cvtColor(preprocessImage, cv2.COLOR_GRAY2BGR) # preprocessImage
    fourth_pic = out_img # out_img
    fifth_pic = cv2.cvtColor(binary_warped, cv2.COLOR_GRAY2BGR)
