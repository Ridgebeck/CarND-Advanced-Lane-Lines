...
class tracker():
	...
	# remember to specify all unanssigned variables
	def __init__(self, Mywindow_width, Mywindow_height, Mymargin, My_ym = 1, My_xm = 1, Mysmooth_factor = 15):
		...
		# the window pixel width of the cneter values, used to count pixels inside center windows to determine curve values
		self.window_width = Mywindow_width

		# the window pixel height of the cneter values, used to count pixels inside center windows to determine curve values
		# breaks the image into vertical levels
		self.window_height = Mywindow_height

		# the picel distance in both directions to slide (left window + right window) template for searching
	...
	def find_window_centroids(self, warped):
		...
		window = np.ones(window_width) # creating the window template thate will be used for convolutions
